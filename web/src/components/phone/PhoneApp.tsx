...
function MiniStatusBar({ time = '9:41' }: { time?: string }) {
  return (
    <div
      style={{
        height: 50,
    ...
    >
      ...
      <span style={{ display: 'inline-flex', gap: 5, alignItems: 'center' }}>
        <svg width="17" height="11" viewBox="0 0 17 11">
          <path
            d="M8.5 0a8.5 8.5 0 016 2.5l-1.5 1.5a6.4 6.4 0 00-9 0L2.5 2.5A8.5 8.5 0 018.5 0zm0 4a4.5 4.5 0 013 1.2l-1.5 1.5a2.4 2.4 0 00-3 0L5.5 5.2A4.5 4.5 0 018.5 4zm0 4a1.5 1.5 0 011.1.4L8.5 9.5 7.4 8.4A1.5 1.5 0 018.5 8z"
            fill="currentColor"
          />
        </svg>
        <svg width="16" height="11" viewBox="0 0 16 11" fill="none" stroke="currentColor" strokeWidth="1">
          <rect x="1" y="2" width="11" height="7" rx="2" />
          <rect x="2.5" y="3.5" width="8" height="4" rx="0.6" fill="currentColor" stroke="none" />
...
function TodayScreen({ onTab }: { onTab: (t: Tab) => void }) {
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {items.map((item, i) => (
              <div
                ...
                style={{
                  display: 'grid',
                  gridTemplateColumns: '46px auto 1fr',
                  gap: 10,
                  alignItems: 'center',
                  padding: '14px 0',
...
function MemoryScreen({ onTab }: { onTab: (t: Tab) => void }) {
            ...
            {[0, 1, 2].map((i) => (
              <span
                ...
                style={{
                  ...
                  animation: `niklausDot 1.4s ease-in-out ${i * 0.2}s infinite`,
                }}
              />
            ))}
            <style>{`@keyframes niklausDot { 0%,80%,100%{opacity:.3;transform:scale(.7)} 40%{opacity:1;transform:scale(1.1)} }`}</style>
          </div>
        )}
...
function HeritageScreen({ onTab }: { onTab: (t: Tab) => void }) {
  const rows = [
    ...
    >
      <div
        style={{
          ...
          padding: 11,
          boxShadow:
            '0 30px 80px -20px rgba(0,0,0,0.35), 0 6px 14px rgba(0,0,0,0.18), inset 0 0 0 1px rgba(255,255,255,0.05)',
          position: 'relative',
        }}
      >
        <span
...
export default function PhoneApp({ initial = 'today', scale = 1, label }: PhoneAppProps) {
